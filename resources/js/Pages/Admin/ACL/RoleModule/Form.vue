...
const addModule = (module) => {
    ...
    filterForm.post(route("admin.role-module.store"), {
        onFinish: () => {
            if (page.props.flash) {
...
const deleteModule = (role_modules) => {
    useForm({}).delete(route("admin.role-module.destroy", role_modules), {
        onFinish: () => {
            if (page.props.flash) {
                showToast(page.props.flash);
                datatable.value?.refreshTable();
...
<template>
    ...
    <AuthenticatedLayout>
        <BaseCard
            ...
            :back_route="route('admin.role.index')"
        >
            <template #content>
                <CompServerSideDataTable
                    ref="datatable"
                    :filter_form="filterForm"
                    :route="route('admin.ajax.role-module.dt-module')"
                >
                    <template #columns>
