...
const addPermission = (permission) => {
    ...
    filterForm.post(route("central.module-permission.store"), {
        onFinish: () => {
            if (page.props.flash) {
...
const deletePermission = (module_permissions) => {
    useForm({}).delete(
        route("central.module-permission.destroy", module_permissions),
        {
            onFinish: () => {
                if (page.props.flash) {
                    showToast(page.props.flash);
...
<template>
    ...
    <AuthenticatedLayout>
        <BaseCard
            ...
            :back_route="route('central.module.index')"
        >
            <template #content>
                <CompServerSideDataTable
                    ...
                    :route="
                        route('central.ajax.module-permission.dt-permission')
                    "
                >
                    <template #columns>
