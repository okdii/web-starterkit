...
const addModule = (module) => {
    ...
    filterForm.post(route("central.role-module.store"), {
        onFinish: () => {
            if (page.props.flash) {
...
const deleteModule = (role_modules) => {
    useForm({}).delete(route("central.role-module.destroy", role_modules), {
        onFinish: () => {
            if (page.props.flash) {
                showToast(page.props.flash);
                datatable.value?.refreshTable();
        ...
        >
            <template #content>
                <CompServerSideDataTable
                    ...
                    :route="route('central.ajax.role-module.dt-module')"
                >
                    <template #columns>
