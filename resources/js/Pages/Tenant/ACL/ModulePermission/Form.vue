...
const addPermission = (permission) => {
    ...
    filterForm.post(route("tenant.module-permission.store"), {
        onFinish: () => {
            if (page.props.flash) {
...
const deletePermission = (module_permissions) => {
    useForm({}).delete(
        route("tenant.module-permission.destroy", module_permissions),
        {
            onFinish: () => {
                if (page.props.flash) {
                    showToast(page.props.flash);
...
<template>
    ...
    <AuthenticatedLayout>
        <BaseCard
            ...
            :back_route="route('tenant.module.index')"
        >
            <template #content>
                <CompServerSideDataTable
                    ...
                    :route="
                        route('tenant.ajax.module-permission.dt-permission')
                    "
                >
                    <template #columns>
